...
class CMENet(nn.Module):
    def __init__(self, config, backbone='CME', drop=0.1):
        ...
        self.classifier_fusion = Class_head(in_channels=96, num_classes=config.num_classes)
    ...
    def forward(self, t1, t1c, tof, stage="seg", img_name="None"):
        ...
        if stage == "seg":  # 分割阶段 只需要分割掩码
            ...
            # 融合分类结果
            fusion_class = self.classifier_fusion(class_inp3)
    ...
    def visualize_attention_simple(self, t1, t1c, tof, stage='cls', save_path='attention_visualization'):
        ...
        with torch.no_grad():
            ...
            if stage == 'seg':
                ...
                # selected_slices = [num_slices//4, num_slices//2, 3*num_slices//4]  # 选择1/4, 1/2, 3/4位置的切片
                
                for slice_idx in num_slices:
                    if slice_idx < num_slices:
                        ...
                        plt.colorbar()
                        plt.title(f'Segmentation Attention - Slice {slice_idx}')
                        plt.savefig(f'{seg_dir}/seg_attention_slice_{slice_idx}.png', 
            ...
            if stage == 'cls':
                ...
                # 融合特征 class_inp3
                fusion_attention = torch.mean(class_inp3[batch_idx], dim=0).detach().cpu().numpy()
                
                plt.figure(figsize=(8, 6))
                plt.imshow(fusion_attention, cmap='plasma', interpolation='nearest')
                plt.colorbar()
