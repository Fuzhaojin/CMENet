...
class CMENet(nn.Module):
    ...
    def _get_segmentation_features(self, t1, stage="seg"):
        ...
        return totalout, mask_out
        

    def forward(self, t1c, stage="seg", img_name="None"):
        ...
        totalout, mask_out = self._get_segmentation_features(t1c)
        ...
        if stage == "seg":  # 分割阶段 只需要分割掩码
            ...
            # 融合分类结果
            fusion_class = self.classifier_fusion(totalout)
