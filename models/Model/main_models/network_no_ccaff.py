...
class CMENet(nn.Module):
    def __init__(self, config, backbone='CME', drop=0.1):
        ...
        self.conv_2 = nn.Sequential(
            DOConv2d(216, 176, kernel_size=1, bias=False),
            nn.BatchNorm2d(176),
            nn.ReLU()
        )
        
        self.conv_1 = nn.Sequential(
            DOConv2d(176, 96, kernel_size=1, bias=False),
            nn.BatchNorm2d(96),
            nn.ReLU()
        )
    ...
    def _get_segmentation_features(self, t1, t1c, tof, stage="seg"):
        ...
        # print(totalout.shape, skip3.shape)  # torch.Size([24, 216, 16, 16]) torch.Size([24, 216, 32, 32])
        # totalout = self.ccaff3(totalout, skip3)  # 跨尺度融合 (4层,3层)
        totalout = totalout + F.interpolate(skip3, size=[hei // 8, wid // 8], mode='bilinear', align_corners=True)
        # print(totalout.shape)  # torch.Size([24, 216, 16, 16])
        ...
        # print(totalout.shape, skip2.shape) # torch.Size([24, 216, 32, 32]) torch.Size([24, 176, 32, 32])
        # totalout = self.ccaff2(totalout, skip2)  # 跨尺度融合 (3层,2层)
        totalout = self.conv_2(totalout) + skip2
        # print(totalout.shape)  # torch.Size([24, 176, 32, 32])
        ...
        # print(totalout.shape, skip1.shape) # torch.Size([24, 176, 64, 64]) torch.Size([24, 96, 64, 64])
        # totalout = self.ccaff1(totalout, skip1)  # 跨尺度融合 (2层,1层)
        totalout = self.conv_1(totalout) + skip1
        # print(totalout.shape)  # torch.Size([24, 96, 64, 64])
