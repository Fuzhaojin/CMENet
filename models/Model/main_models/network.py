...
class CMENet(nn.Module):
    ...
    def _get_segmentation_features(self, t1, t1c, tof, stage="seg"):
        ...
        # 可视化专用 
        # return (c4, a4, e4), out, (class_inp0, class_inp1, class_inp2, class_inp3)
    ...
    def forward(self, t1, t1c, tof, seg_result, stage="seg", img_name="None"):
        ...
        if stage == "seg":  # 分割阶段 只需要分割掩码
            ...
            t1_class = self.classifier_t1(c4, seg_result)
            t1c_class = self.classifier_t1c(a4, seg_result)
            # t2_class = self.classifier_t2(d4, seg_result)
            tof_class = self.classifier_tof(e4, seg_result)
            # 融合分类结果
            fusion_class = self.classifier_fusion(class_inp0, seg_result)
    ...
    # def visualize_attention_simple(self, t1, t1c, tof, stage='cls', save_path='attention_visualization'):
    #     """简化版注意力可视化 - 只保存纯净的图像，无标注"""
    #     import os
    #     import matplotlib.pyplot as plt
    #     import numpy as np
    ...
    #     with torch.no_grad():
    #         # 首先可视化输入数据
    #         # self._visualize_input_data(t1, t1c, tof, save_path)
    ...
    #                 plt.savefig(f'{seg_dir}/seg_mask_slice_{slice_idx:02d}.png', 
    #                         bbox_inches='tight', pad_inches=0, dpi=100)
    #                 plt.close()
                
    #             print(f'分割任务: 已生成 {num_slices} 个切片的注意力图')
    #         # import pdb;pdb.set_trace()
    #         if stage == 'cls':
    #             # 2. 分类任务热图 - 关注 (c4, a4, e4) 和 pred
    ...
    #             # 融合特征 pred - 处理所有切片
    #             pred_dir = os.path.join(cls_dir, 'fusion_slices')
    #             os.makedirs(pred_dir, exist_ok=True)
                
    #             num_pred_slices = class_inp0.shape[1]
    #             # pdb.set_trace()
    #             for slice_idx in range(num_pred_slices):
    #                 pred_slice = class_inp0[batch_idx, slice_idx].detach().cpu().numpy()
                    
    #                 # 统一使用相同的尺寸处理方式
    #                 height, width = pred_slice.shape
    #                 plt.figure(figsize=(width/100, height/100), dpi=100)
    #                 plt.imshow(pred_slice, cmap='plasma', interpolation='nearest')
    #                 plt.axis('off')
    #                 plt.gca().set_position([0, 0, 1, 1])
    #                 plt.savefig(f'{pred_dir}/cls_fusion_slice_{slice_idx:02d}.png', 
    #                         bbox_inches='tight', pad_inches=0, dpi=100)
    #                 plt.close()
                
    #             print(f'分类任务: 已生成 {num_pred_slices} 个融合特征切片的注意力图')
            
    #         print(f'注意力热图已保存到 {save_path}/')

    # def _visualize_input_data(self, t1, t1c, tof, save_path):
    #     """可视化输入的三种模态数据"""
    #     import os
    #     import matplotlib.pyplot as plt
        
    #     input_dir = os.path.join(save_path, 'input_data')
    #     os.makedirs(input_dir, exist_ok=True)
        
    #     batch_idx = 0
        
    #     # 三种模态数据
    #     modalities = [t1, t1c, tof]
    #     modality_names = ['T1', 'T1C', 'TOF']
        
    #     for modality_data, modality_name in zip(modalities, modality_names):
    #         # 为每种模态创建子目录
    #         modality_dir = os.path.join(input_dir, modality_name)
    #         os.makedirs(modality_dir, exist_ok=True)
            
    #         # 获取切片数量
    #         num_slices = modality_data.shape[1]
            
    #         print(f"正在保存 {modality_name} 的 {num_slices} 个切片...")
            
    #         # 处理所有切片
    #         for slice_idx in range(num_slices):
    #             slice_data = modality_data[batch_idx, slice_idx].detach().cpu().numpy()
    ...
    #             plt.axis('off')
    #             plt.gca().set_position([0, 0, 1, 1])
    #             plt.savefig(f'{modality_dir}/{modality_name}_slice_{slice_idx:02d}.png', 
    #                     bbox_inches='tight', pad_inches=0, dpi=100)
    #             plt.close()
            
    #         print(f'{modality_name}: 已生成 {num_slices} 个切片')
