...
# DCAB (Dilated Context Attention Block)
...
# PSGP (Pyramid Split Global Pooling)
class PyramidSplitGlobalPooling(nn.Module):
...
# SSSA Block (Split Semantic Selector Aggregator)
class SplitSemanticSelectorAggregator(nn.Module):
    def __init__(self, Ch, h, window):
        ...
        self.psgp = PyramidSplitGlobalPooling(Ch * h)
    ...
    def forward(self, q, v, size):
        ...
        psgp = rearrange(psgp, "B (h Ch) H W -> B h (H W) Ch", h=h)

        dynamic_filters = q * lsss + psgp
        return dynamic_filters

# SESAM (Semantic-Strength Augmented Multi-head Attention)
class Semantic_StrengthAugmentedMulti_headAttention(nn.Module):
...
# SST Block (Semantic Strength Transformer Block)
class SSTBlock(nn.Module):
    def __init__(
        ...
        self.att_result = Semantic_StrengthAugmentedMulti_headAttention(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
    ...
    def forward(self, x, size):
        if self.pe is not None:
            x = self.pe(x, size)
        cur = self.norm1(x)
        x = x + self.drop_path(self.att_result(cur, size))    # 消融实验

        cur = self.norm2(x)
        x = x + self.drop_path(self.mlp(cur, size))
        return x

# SSTA (Semantic Strength Transformer Aggregator)
class SemanticStrengthTransformerAggregator(nn.Module):
    def __init__(
            self,
            dim,
            num_layers=1,
        ...
        self.sssa = SplitSemanticSelectorAggregator(Ch=dim // num_heads,
                                  h=num_heads,
                                  window=crpe_window)
        self.SST_layers = nn.ModuleList([
            SSTBlock(
                dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
                ...
                qk_scale=qk_scale,
                shared_cpe=self.pe,
                semantic_strength_selector=self.sssa,
...
# DRB Block (Dynamic Reconstruction Block)
class DynamicReconstructionBlock(nn.Module):
...
class PM(nn.Module):
    def __init__(
        ...
        self.drb = DynamicReconstructionBlock(in_features=embed_dim, out_features=embed_dim)
        self.DCAB = DCAB(in_planes=embed_dim,out_planes=embed_dim)

        if id_stage > 0:
            self.aggregate = Conv2d_BN(embed_dim * (num_path),
                                       out_embed_dim,
                                       act_layer=nn.Hardswish)
            self.ssta = nn.ModuleList([
                SemanticStrengthTransformerAggregator(
                    embed_dim,
                    num_layers,
                    num_heads,
                    mlp_ratio,
    ...
    def forward(self, inputs, id_stage):
        ...
        if id_stage > 0:
            inputs[0] = self.DCAB(inputs[0])  # DCAB  消融实验

            att_outputs = [self.drb(inputs[0])]
            for x, encoder in zip(inputs[1:], self.ssta):
                _, _, H, W = x.shape
            ...
            out_concat = self.drb(inputs[0] + inputs[1])
